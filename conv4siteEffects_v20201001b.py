...
hdf5FileName = 'calc_2020925.hdf5'
...
fileNamesAccInp = glob.glob(path2accTHist+'\\*0.00*accelTs*')
timeHistorInp = list()
for j in fileNamesAccInp:
    timeHistorInp.append(np.genfromtxt(j,delimiter=',', skip_header=3))
# Dito
fileNamesAccOut = glob.glob(path2accTHist+'\\*Bedrock*accelTs*')
timeHistorOut = list()
...
#%%  
# Differentiate the rock-hazard curves. This is required in Equation 3.
# The paper says
...
# If this does not work, try differentiating the complement of the hazard curve
# (1-P). The complement should look like a CDF and the differentiation should
# give something like a PDF. The pX(xj) in the paper looks more like a PDF.
...
#    diffExProb4hazCurves.append(np.gradient(exProb4hazCurves[j].copy(),
#                        imLevels4hazCurves[j]))
...
funGeq4 = list()
for j in range(len(periods4hazCurves)):
    xValues = np.zeros(len(timeHistorInp))
    yValues = ampliFun[j]
    funGeq4.append(list())
    for n in range(len(timeHistorInp)):
        xValues[n] = respSpectraInp[n][j].copy()
    for m in range(len(imLevels4hazCurves[j])):
        ...
        p = np.polyfit(np.log(xValues), np.log(yValues), 1);
        c = p[0];
        lnb = p[1];
        lnD = lnb + c * np.log(xValues);
        epsilon = np.log(yValues) - lnD;
        dispersion = np.std(epsilon) / c;
        b = np.exp( lnb );
        medians = np.exp( np.log( yThresholds / b ) / c );
        funGeq4[-1].append(norm.sf(np.log(imLevels4hazCurves[j][m]),
               np.log(medians), dispersion))
        # Using few data can lead to negative c values and then to NaNs
#        if c <= 0:
#            funGeq4[-1][-1] = np.ones(len(imLevels4hazCurves[j]))



#%%  
# Do the convolution to compute the new hazard curves
# By convolving the survival function in Equation 4 with the derivative of the
# complement of the hazard curve we get the probability to exceed the IM. The
# complement of this probability is the new hazard curve.
...
for j in range(len(periods4hazCurves)):
    ...
    # We used 2nd order differences for the gradient
